        ...
        <div className="flex items-center gap-3 sm:gap-4">
          {searchOpen ? (
            <form className="flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); if (query.trim()) window.location.href = `/${locale === "nl" ? "" : locale + "/"}activiteiten?q=${encodeURIComponent(query.trim())}`; }}>
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("searchPlaceholder")} autoFocus className="w-36 sm:w-48 rounded-full border border-[#F4A09C] bg-white px-3 py-1.5 text-[13px] outline-none" />
            ...
            <>
              ...
              <span className="flex items-center rounded-full border border-[#E8E0D8] text-[11px] font-bold">
                <Link
                  href={pathname}
                  locale="nl"
                  className={`rounded-full px-2.5 py-1 transition-colors ${locale === "nl" ? "bg-[#F4A09C] text-white" : "text-[#6B6B6B] hover:text-[#2D2D2D]"}`}
                >
                  NL
                </Link>
                <Link
                  href={pathname}
                  locale="en"
                  className={`rounded-full px-2.5 py-1 transition-colors ${locale === "en" ? "bg-[#F4A09C] text-white" : "text-[#6B6B6B] hover:text-[#2D2D2D]"}`}
                >
                  EN
                </Link>
              </span>
